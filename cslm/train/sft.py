...
from transformers import LlamaForCausalLM, AutoTokenizer, HfArgumentParser, DataCollatorForSeq2Seq
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    local_rank: int = field(
        default=-1,
        metadata={"help": "local_rank"}
    )
    pad_token_id: Optional[int] = field(
        default=None,
        metadata={"help": "Optional pad token id. If not set, uses tokenizer default or eos token id."}
    )
...
def train():
    ...
    os.makedirs(os.path.join(training_args.cache_dir, "tokenized", "train"), exist_ok=True)
    os.makedirs(os.path.join(training_args.cache_dir, "tokenized", "valid"), exist_ok=True)
    ...
    # Default llama3 pad id is 128002; can be overridden by --pad_token_id.
    tokenizer.pad_token_id = (
        training_args.pad_token_id
        if training_args.pad_token_id is not None
        else (tokenizer.pad_token_id if tokenizer.pad_token_id is not None else 128002)
    )
            ...
            for i, token_id in enumerate(tokenized_prompt["input_ids"]):
                ...
                if token_id == seg_list[0] and tokenized_prompt["input_ids"][i:i+len(seg_list)] == seg_list:
                    if i + len(seg_list) >= len(tokenized_prompt["input_ids"]):
                        logger.warning(f"seg_list exceeds input_ids length, i={i}")
                        break
                    input_end = i
    ...
    if training_args.val_set_size > 0:
        ...
        train_data = data["train"].map(
                ...
                num_proc=training_args.preprocessing_num_workers,
                load_from_cache_file=True,
                cache_file_name=tokenized_cache_file_names["train"],
                desc=f"generate_and_tokenize_prompt",
                )
        val_data = None
